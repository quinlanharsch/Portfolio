...
def fitness(p, sz):  # note: can be run with {x,s} or {x}
     fits = []
     for i in range(sz):
         fits.append(-20 * np.exp(-0.2 * np.sqrt((1.0 / n) * np.sum(np.square(p[i,:n]), 0, float))) \
         - np.exp((1.0 / n) * np.sum(np.cos(2 * np.pi * p[i, :n]), 0, float)) + 20 + np.e)
...
def crossover(p) -> object:
    ...
    for i in range(c_size):
        ...
        if x_fit[0] < x_fit[1]: sel_x = x[0]  # pick the best (lowest fitness)
        else: sel_x = x[1]
...
def survivors(children): return sort(children)[:mu]
